function stBessel = sphGetXiPsiNormal(nNmax, s, x, NB)
  %% sphGetXiPsi
% Calculates normal (unmodified) Bessel function products for spheroids
% 
% sphGetXiPsi(nNmax, s, x, NB) calculates the products
% xi(n,x)*psi(k,sx) and psi(n,x)*psi(k,sx) for n,k from 0 to nNmax+1. 
% NB>=N multipoles are used for the calculation (for improved precision).
%
...
%             - xipsi: [N+2 x N+2 x T] The product xi_n psi_k
...
psik=vshRBpsi(0:(nNmax+1), s.*x); % [T x K+1]
chin=vshRBchi(0:(nNmax+1), x); % [T x N+1]
...
chipsi = zeros(nNmax+1, nNmax+1, length(x));
...
for nInd=1:(nNmax+2)
    ...
    chipsi(nInd, kInds, :) = transpose( ...
        bsxfun(@times, psik(:, kInds), chin(:, nInd))); % 1 x K/2 x X
end
